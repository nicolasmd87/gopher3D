...
// NewGLFWFromExistingWindow creates a GLFW platform using an existing window
func NewGLFWFromExistingWindow(window *glfw.Window, io imgui.IO) (*GLFW, error) {
	runtime.LockOSThread()
...
func (platform *GLFW) Dispose() {
	// Only destroy/terminate if we created the window ourselves
	// For editor, we don't own the window
}
