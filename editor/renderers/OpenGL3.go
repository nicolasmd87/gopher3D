...
import (
	...
	"Gopher3D/editor/renderers/gl/v3.2-core/gl"
...
func NewOpenGL3(io imgui.IO) (*OpenGL3, error) {
	// Initialize OpenGL function pointers for THIS binding
	// (This is a different binding than the engine's, so we need to init it)
	// Safe to call here since we're on main thread with active GL context
	err := gl.Init()
	if err != nil {
		return nil, fmt.Errorf("failed to initialize OpenGL bindings: %w", err)
	}
